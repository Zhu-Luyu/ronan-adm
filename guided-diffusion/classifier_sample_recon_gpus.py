...
def main(rank, world_size, experiment_folder_path, args):
    # 1. 初始化分布式环境
    setup(rank, world_size) 
    
    # 2. 根据rank设置当前进程使用的GPU
    ...
    for i in range(args.num_iter):
        ...
        while len(all_images) * args.batch_size < args.num_samples:
            ...
            if args.mixed_precision:
                ...
                # 保存每一次重建的图像
                step_num_str=str(i)
                file_name = f"image_rank{rank}_{step_num_str}_{args.input_selection}_{args.distance_metric}_{str(args.lr)}.png"
                image1 = (sample + 1) / 2
...
if __name__ == "__main__":
    ...
    mp.spawn(main, args=(world_size, experiment_folder_path, args), nprocs=world_size, join=True)
